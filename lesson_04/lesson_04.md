...
Якщо ви все зробили вірно, то у вас буде напис `forked from`:
...
Переходите у папку з кодом `cd folder_name` де folder_name - назва вашої папки
...
Щоб переключитися на віддалену гілку виконайте команду `git checkout branch_name`

### Оновленя коду форку
Оскільки код у гілці `main` періодично оновлюється, у вас може виникнути необхідність отримати новішу версію коду.
...
Для оновленя коду:
1. Перемикнутися на гілку, що треба оновити, напр. `git checkout main`
2. Перевірити, що робоче дерево чисте.
3. Втягнути зміни: `git pull upstream main`
